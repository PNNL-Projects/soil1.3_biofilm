...
for rep_samples, folder in zip([rep1_samples, rep2_samples, rep3_samples], ["results_rep" + str(i+1) for i in range(3)]):
    ...
    for sample in rep_samples:
        ...
        with open(rgb_file, 'rb') as f:
            rgb_array = pickle.load(f)
        pop_vol = slide_sum_y_vol(rgb_array, slide_len = 1)
        for i in range(len(pop_vol)):
            pop_volumes_1d.append([folder.split('_')[1], sample[1], sample[2], pop_vol[i,0], pop_vol[i,1], pop_vol[i,2], i])
            if i > 0:
                delta_pop_volumes_1d.append([folder.split('_')[1], sample[1], sample[2], pop_vol[i,0] - pop_vol[i-1,0], pop_vol[i,1] - pop_vol[i-1,1], pop_vol[i,2] - pop_vol[i-1,2], i, 1])
